...
class ChatService(
    ...
    fun handleStopSelection(stop: RouteStop) {
        ...
        val stopDetailData = TransitData.StopDetailData(
            stop = Stop(
                id = stop.stopId,
                name = stop.stopName,
                location = Location(
        ...
        chatRepository.addAutogeneratedMessage(autogeneratedText, QueryType.STOPDETAIL)

        // Add BOT message
        val botContent = stop.stopName
        chatRepository.addBotMessage(
            content = botContent,
            queryType = QueryType.STOPDETAIL,
            data = stopDetailData,
            isCompact = false
        )
    }

    private fun getRouteDetailData(routeId: String): TransitData {
