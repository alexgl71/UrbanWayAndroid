...
class ChatService(
    ...
    fun handleModalSelection(selectedRoute: String, fromQueryType: QueryType) {
        when (fromQueryType) {
            QueryType.NEARBY -> {
                // User selected a route from nearby results (e.g., "56 - TABACCHI")
                // Extract the route number/ID from the selection
                val routeId = extractRouteId(selectedRoute)

                // Get route detail data
                val routeData = getRouteDetailData(routeId)

                // Generate autogenerated message using QueryParser logic
                val autogeneratedText = queryParser.generateAutogeneratedText(selectedRoute, QueryType.ROUTEDETAIL)
